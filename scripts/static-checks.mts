...
async function validateSuppressions(context: CheckContext): Promise<CheckOutcome> {
  ...
  problems.push(...(await inlineDirectives(context, known)));
 ...
 * pruned; an `eslint-disable` comment is the one way to silence one that leaves
 * nothing behind to review. A file with no entry can acquire a directive and
 * pass every other check here, so the diff's own sources are read for one.
 */
async function inlineDirectives(context: CheckContext, known: Set<string>): Promise<string[]> {
  const problems: string[] = [];
  for (const file of context.sourceFiles) {
    if (!DESIGN_SOURCE.test(file)) continue;
    if (!DESIGN_ROOTS.some((root) => file.startsWith(`${root}/`))) continue;
    let source: string;
    try {
    ...
    /** `eslint-disable` is one of two spellings; `/* eslint rule: off *\/` is
     *  configuration rather than a disable comment and silences just as well. */
    for (const match of source.matchAll(
      /eslint-disable(?:-next-line|-line)?\s+([^*\n]+)|\beslint\s+((?:[\w@/-]+\s*:\s*(?:\d|"|'|\[|off|warn|error)[^,*\n]*,?\s*)+)/g,
    )) {
      const body = match[1] ?? match[2] ?? '';
      const named = (body.match(/[\w@/-]+\/[\w-]+/g) ?? [])
        .map((rule) => rule.trim())
    ...
    if (/eslint-disable(?:-next-line|-line)?\s*(?:\*\/|$)/m.test(source)) {
      problems.push(
        `${file}: a blanket eslint-disable turns the design rules off with everything else; name the rules you mean, and record design-rule debt in ${SUPPRESSIONS_FILE}`,
      );
    }
  }
  return problems;
