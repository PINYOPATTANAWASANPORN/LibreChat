...
test.describe('the recorded design-rule backlog', () => {
  ...
  test('a suppressions-only change is selected and validated by static checks @scenario:a-suppressions-only-change-is-selected-and-validated-by-static-checks', () => {
    ...
    /** A directive is the one way to silence a design rule without leaving a
     *  count behind, so the check reads the diff's own sources for one — a file
     *  with no entry could otherwise pass everything else. */
    ...
    try {
      ...
      const disabled = staticChecks([
        'client/src/__directive_probe__.tsx',
        '--only',
        'suppressions',
      ]);
      expect(disabled.status, 'an inline disable of a design rule passed').not.toBe(0);
      expect(disabled.output).toContain('shadcn/no-raw-colors is silenced by an inline comment');

      /** And the other spelling: rule configuration rather than a disable. */
      writeFileSync(
        directive,
        '/* eslint shadcn/no-raw-colors: off */\nexport default () => <div className="bg-pink-500" />;\n',
      );
      const configured = staticChecks([
        'client/src/__directive_probe__.tsx',
        '--only',
        'suppressions',
      ]);
      expect(configured.status, 'an inline rule configuration passed').not.toBe(0);
      expect(configured.output).toContain('shadcn/no-raw-colors is silenced by an inline comment');

      /** And a directive naming something else is nobody's business here. */
      writeFileSync(
        directive,
        '// eslint-disable-next-line react-hooks/exhaustive-deps\nexport default () => <div className="bg-surface-primary" />;\n',
      );
      const unrelated = staticChecks([
        'client/src/__directive_probe__.tsx',
        '--only',
        'suppressions',
      ]);
      expect(unrelated.status, unrelated.output).toBe(0);
    } finally {
